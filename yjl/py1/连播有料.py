...
class Spider(BaseSpider):
    ...
    def _full(self, url):
        ...
        if ".m3u8" not in u:
            return u
        i = u.rfind("/")
        name = u[i + 1:].split("?")[0] if i >= 0 else u
        if name.startswith("index"):
            return u[:i + 1] + "index.m3u8" if i > 0 else u
        txt = self._fetch_text(u, None, 10)
        if txt and "#EXT-X-STREAM-INF" in txt:
            tgt = ""
            for line in txt.replace("\r", "").split("\n"):
                ...
                if line and not line.startswith("#"):
                    ...
                    break
            if tgt:
                vurl = self._abs(u[:i + 1] if i > 0 else u, tgt)
                j = vurl.rfind("/")
                cand = vurl[:j + 1] + "index.m3u8" if j > 0 else ""
                if cand:
                    ct = self._fetch_text(cand, None, 10)
                    if ct and "#EXTINF" in ct:
                        return cand
        return u
    ...
    def homeContent(self, filter):
        ...
        cls = [{"type_id": self.YL, "type_name": "有料"}, {"type_id": self.DY, "type_name": "抖阴"},
               {"type_id": self.LIVE, "type_name": "直播"}, {"type_id": self.NEW, "type_name": "最新"}]
    ...
    def _filters(self, cls):
        ...
        f[self.DY] = self._dy_filters()
        f[self.LIVE] = [{"key": "live", "name": "子类", "value": self._live_subs()}]
        for c in cls:
            tid = c["type_id"]
            if tid in (self.NEW, self.LIB, self.YL, self.DY, self.LIVE):
                continue
            subs = self._subs(tid)
            if not subs:
                continue
    ...
    def _dy_cates(self):
        if self.dy_cates:
            return self.dy_cates
        d = self._call("/short_video/short_cate", {"page": 1, "page_size": 50}).get("data") or {}
        lst = d.get("list") if isinstance(d, dict) else []
        out = []
        for c in lst or []:
            if not isinstance(c, dict) or not c.get("cate_id"):
                continue
            out.append({"n": str(c.get("name") or c["cate_id"]), "v": str(c["cate_id"])})
        self.dy_cates = out
        return out

    def _dy_filters(self):
    ...
    def _item_id(self, it):
        return str((it or {}).get("video_id") or (it or {}).get("id") or "")

    def _item_title(self, it):
        return str((it or {}).get("title") or (it or {}).get("video_title") or (it or {}).get("name") or "")

    def _item_pic(self, it):
        return (it or {}).get("img") or (it or {}).get("video_img") or (it or {}).get("cover") or (it or {}).get("icon") or ""

    def _item_url(self, it):
        u = str((it or {}).get("url") or (it or {}).get("video_url") or "").strip()
        if u.startswith("http"):
            return u
        iid = self._item_id(it)
        return "nid:%s" % iid if iid else ""

    def _raw_videos(self, items):
        seen, out = set(), []
        for it in items or []:
            if not isinstance(it, dict):
                continue
            vid = self._item_id(it)
            name = self._item_title(it)
    ...
    def _dy_map(self, items):
        out = []
        for it in self._raw_videos(items):
            rem = str(it.get("time_len") or "")
            pn = it.get("play_num")
            if pn:
                rem = ("%s %s" % (rem, self._num(pn))).strip()
            out.append({"vod_id": "s_%s" % self._item_id(it), "vod_name": self._item_title(it),
                        "vod_pic": self._img(self._item_pic(it)), "vod_remarks": rem})
        return out
    ...
    def _dy_page(self, cate, pg):
        pg = max(int(pg or 1), 1)
        cate = str(cate or "").strip()
        uid = self.uid
        if not cate:
            items = []
            a1 = (pg - 1) * 2 + 1
            for ap in (a1, a1 + 1):
                d = self._call("/short_video/short_video_list", {"page": ap, "page_size": 20, "uid": uid}).get("data") or {}
                items += d.get("list") or []
            self._short_cache_put("rec", pg, items)
            vl = self._dy_map(items)
            pc = pg + 1 if len(vl) >= 20 else pg
            return {"list": vl, "page": pg, "pagecount": pc, "limit": 40, "total": pc * 40}
        r = self._call("/short_video/short_cate_list", {"cate_id": cate, "page": pg, "page_size": 36, "uid": uid})
        d = r.get("data") or {}
        info = d.get("list_info") if isinstance(d.get("list_info"), dict) else d
        items = (info or {}).get("list") or []
        self._short_cache_put("c_" + cate, pg, items)
        vl = self._dy_map(items)
    ...
    def _tags_text(self, it):
        ...
        return str(tags or (it or {}).get("author_name") or "")
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if tid == self.NEW:
            ...
            return self._page(items, pg, 12, "new")
        if tid == self.LIB:
            ...
            return self._page(items, pg, 20, "lib")
        if tid == self.YL:
            return self._yl_page(str(ext.get("yl") or "gossip").strip() or "gossip", pg)
        if tid == self.DY:
            return self._dy_page(str(ext.get("dy2") or ext.get("dy1") or "").strip(), pg)
        if tid == self.LIVE:
        ...
        if not plan:
            ...
            return self._page(items, pg, 42, "t_%s" % tid)
        per = 2
        total = len(plan)
    ...
    def detailContent(self, ids):
        ...
        if vid.startswith("s_"):
            got = self._play_detail(vid[2:], vod_id=vid, type_name="抖阴")
            if got:
                return got
            return {"list": [{"vod_id": vid, "vod_name": vid[2:], "vod_pic": "", "type_name": "抖阴",
                              "vod_remarks": "", "vod_year": "", "vod_area": "", "vod_actor": "", "vod_director": "",
                              "vod_content": "", "vod_play_from": "线路1", "vod_play_url": "播放$nid:%s" % vid[2:]}]}
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if s.startswith("nid:"):
            nid = s[4:]
            for it in self._short_cache_get(nid) or []:
                if self._item_id(it) == nid:
            ...
            if not url:
                for line in (vi.get("video_line") or []):
                    if line.get("url"):
                        url = str(line["url"])
                        break
            return {"parse": 0, "playUrl": "", "url": self._full(url), "header": self._hdr()}
        if s.startswith("g_"):
