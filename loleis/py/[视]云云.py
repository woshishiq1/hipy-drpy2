...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        body = f"vod_id={ids[0]}"
        data = self.getdata("/api.php/qijiappapi.index/vodDetail", body)
        vod = data["vod"]
        play = []
        names = []
    ...
    def searchContent(self, key, quick, pg="1"):
        body = f"keywords={key}&type_id=0&page={pg}".encode('utf-8')
    ...
    def gethost(self):
        ...
        response = self.fetch('https://staraugust123456.oss-cn-hangzhou.aliyuncs.com/2.txt',headers=headers).text
        return response.strip()

    def aes(self, text, b=None):
        key = b"staraugust123456"
