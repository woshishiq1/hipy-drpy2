...
class Spider(BaseSpider):
    ...
    def __init__(self):
        ...
        self.channels = {
            ...
            'cn': {'name': '国产', 'path': '/category/cn.html'},
            'jav': {'name': '日本', 'path': '/category/jav.html'},
            'asia': {'name': '亚洲', 'path': '/category/asia.html'},
        }
    ...
    def _parse_list(self, html):
        ...
        for m in re.finditer(
            r'href="((?:https?://[^"/]+)?/video/(\d+)\.html)"[^>]{0,280}(?:title|alt)="([^"]*)"',
            html or '',
            re.I,
        ):
            ...
            seen.add(vid)
            videos.append({
                'vod_id': '/video/%s.html' % vid,
                'vod_name': title or vid,
                'vod_pic': '',
                'vod_remarks': vid,
            })
        if not videos:
            for m in re.finditer(r'href="(/video/(\d+)\.html)"', html or ''):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            ...
            url = self.siteUrl + info.get('path', '/')
            if pg > 1:
    ...
    def searchContentPage(self, key, quick, pg=1):
        ...
        try:
            ...
            url = '%s/search/%s.html' % (self.siteUrl, q)
            if pg > 1:
                url = '%s/search/%s-%s.html' % (self.siteUrl, q, pg)
            html = self.fetch_text(url)
            videos = self._parse_list(html)
        except Exception as e:
            print('搜索失败: %s' % e)
