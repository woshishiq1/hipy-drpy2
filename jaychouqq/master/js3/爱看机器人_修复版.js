...
let KSearchNext = {q: '', url: ''};

const getImgProxyDomain = () => (Math.random() < 0.5 ? 'imgp1.ikanbot.eu.org' : 'imgp2.ikanbot.eu.org');
const encPart = s => encodeURIComponent(String(s ?? ''));
const decPart = s => { try { return decodeURIComponent(String(s)); } catch (e) { return String(s); } };
function htmlUnescape(str) {
    try {
        return String(str ?? '').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/gi, ' ');
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        let cateUrl = tid === 'billboard' ? `${HOST}/billboard.html` : `${HOST}/hot/index-${tid}-${encodeURIComponent(fl.class || '热门')}${suffix}.html`;
...
async function search(wd, quick, pg) {
    try {
        ...
        let searchUrl;
        if (pg === 1) {
            KSearchNext = {q: wd, url: ''};
            searchUrl = `${HOST}/search?q=${encodeURIComponent(wd)}`;
        } else if (KSearchNext.q === wd && KSearchNext.url) {
            searchUrl = KSearchNext.url;
        } else {
            searchUrl = `${HOST}/search?q=${encodeURIComponent(wd)}&p=${pg}`;
        }
        let resHtml = await request(searchUrl);
        let VODS = getVodList(resHtml, true);
        let nextHref = '';
        let pmDiv = cutStr(resHtml, 'page-more">', '</div>', '', false);
        if (pmDiv.includes('下一页')) {
            nextHref = cutStr(pmDiv, 'href="', '"', '', false);
            if (nextHref) {
                if (!/^https?:/i.test(nextHref)) { nextHref = HOST + nextHref; }
                nextHref = htmlUnescape(nextHref);
                KSearchNext.url = nextHref;
            }
        }
        let limit = VODS.length;
...
function getVodList(khtml, sch = false) {
    try {
        ...
        let picProxy = 'https://' + getImgProxyDomain() + '/proxy?url=';
        for (let it of listArr) {
            let kname = sch ? cutStr(it, 'title-text">', '<', '名称') : cutStr(it, 'alt="', '"', '名称');
            let rawPic = cutStr(it, 'data-src="', '"', '');
            let kpic = rawPic && /^https?:/i.test(rawPic) ? `${picProxy}${encodeURIComponent(rawPic)}` : '';
            let kremarks = sch ? cutStr(it, '[', ']', '状态') : '无状态';
            let kid = cutStr(it, 'href="', '"');
            if (kid && kname && kname !== '名称') {
                kvods.push({
                    vod_name: kname,
                    vod_pic: kpic,
                    vod_remarks: kremarks,
                    vod_id: `${encPart(kid)}@${encPart(kname)}@${encPart(kpic)}@${encPart(kremarks)}`
                });
            }
        }
        return kvods;
...
async function detail(ids) {
    try {
        let idParts = String(ids ?? '').split('@');
        let id = decPart(idParts[0] || '');
        let kname = decPart(idParts[1] || '');
        let kpic = decPart(idParts[2] || '');
        let kremarks = decPart(idParts[3] || '');
        ...
        resList.forEach((item) => {
            lineNo += 1;
            if (!item || typeof item.resData !== 'string' || !item.resData.trim()) { return; }
            let resData = safeParseJSON(item.resData.replace(/#{2,}/g, '#'))?.[0];
            if (!resData) { return; }
            let url = typeof resData.url === 'string' ? resData.url.trim() : '';
            if (!url || url === 'noUrl') { return; }
            let tab = resData.flag || `线路${lineNo}`;
