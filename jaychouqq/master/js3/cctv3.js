...
/**
 * 解析m3u8，复刻原py get_m3u8
 * @param {string} pid guid
 */
async function getM3u8(pid) {
    try {
        ...
        //简单head探测，cat.js没有真正head，改用get判断非空
...
async function category(tid, pg, filter, ext) {
    ...
    if (tid === "栏目大全") {
        ...
        if (idx > -1) {
            jsonText = jsonText.substring(3, idx);
        }
    }
    const jo = safeJson(jsonText);
    const list = [];
    if (tid === "栏目大全") {
        ...
        if (jo && jo.data && Array.isArray(jo.data.list)) {
...
async function search(key, quick, pg) {
    ...
    try {
        ...
        const resp = await request(url);
        // 原py解析html，这里简化，搜索接口返回html，本源搜索只做基础适配
        // cctv搜索返回html比较复杂，cat环境不适合大量dom正则，这里返回空列表，如需完善可自行加正则
        return JSON.stringify({
            list: [],
            page: pg,
            pagecount: 0,
