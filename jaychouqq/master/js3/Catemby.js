...
function cacheSet(key, value, ttl) {
    if (ttl <= 0) return;
    _cache[key] = { value: value, expire: Date.now() + ttl * 1000 };
}
...
function packPlayId(payload) {
    try {
        ...
        const b64 = Crypto.enc.Base64.stringify(Crypto.enc.Utf8.parse(raw))
            .replace(/\+/g, '-')
            .replace(/\//g, '_')
            .replace(/=+$/, '');
        return PLAY_PREFIX + b64;
    } catch (e) {
        return '';
    }
}
...
function extractBtih(value) {
    ...
    if (/^(?:[A-F0-9]{40}|[A-Z2-7]{32})$/i.test(text.trim())) {
...
function movieCard(raw) {
    const movieId = cleanText(raw.id);
...
function pageResult(rawMovies, page, expectedLimit) {
    ...
    for (const raw of list) {
        ...
        const mid = cleanText(raw.id);
        if (!mid || seen.has(mid)) continue;
        // 简单过滤：有资源才显示
        const hasRes =
            raw.can_play ||
            raw.has_preview_video ||
            raw.preview_video_url ||
            raw.play_sources ||
            numberVal(raw.magnets_count) > 0;
        if (!hasRes) continue;
        seen.add(mid);
...
async function homeVod() {
    try {
        ...
        const result = pageResult(data.movies || [], 1, 24);
...
async function category(tid, pg, filter, ext) {
    ...
    try {
        ...
        let sortBy = 'watched_count';
        let tagId = '';
        if (ext && typeof ext === 'object') {
            if (ext.sort) sortBy = String(ext.sort);
            if (ext.tag) tagId = String(ext.tag).trim();
        }

        const filterBy = tagId
            ? contentType + ':t:' + tagId + '::::'
            : contentType + ':t:::::';
        ...
        const result = pageResult(data.movies || [], pg, 24);
...
async function detail(vodId) {
    try {
        ...
        const detail = await apiGet(
            '/v4/movies/' + encodeURIComponent(movieId),
            {},
            DETAIL_CACHE_TTL
        );
        ...
        // 1. 直连 / 试看（如果源站标记可播）
        if (movie.can_play && number) {
            // 简化：把番号打包，播放时再解析
            const autoId = packPlayId({
                kind: 'auto',
                code: number,
                mode: 'quality',
                declared_duration: movie.duration
            });
            if (autoId) {
                playFromList.push('智能线路');
                playUrlList.push('画质自动$' + autoId + '#极速自动$' + packPlayId({
                    kind: 'auto',
                    code: number,
                    mode: 'speed',
                    declared_duration: movie.duration
        ...
        for (const item of magnets.slice(0, 30)) {
            ...
            magnetItems.push(label + '$' + packPlayId({ kind: 'magnet', magnet: magnet, title: label }));
        }
        ...
        if (playFromList.length === 0) {
            ...
            playUrlList.push(
                ...
                packPlayId({
                    kind: 'error',
                    code: 'no_resources',
                    message: '源站当前没有直连或磁力资源，请使用客户端全局搜索番号 ' + number
                })
            );
        }
        ...
        if (Array.isArray(movie.actors) && movie.actors.length) {
            contentParts.push('女优: ' + movie.actors.map(a => cleanText(a.name || a)).filter(Boolean).join(', '));
        }
...
async function play(flag, playId, flags) {
    try {
        ...
        if (kind === 'magnet') {
            ...
            if (!magnet) {
                return JSON.stringify({ parse: 0, url: '', header: {}, msg: '磁力无效' });
            }
            // 直接返回 magnet，由播放器或下载器处理
            return JSON.stringify({
                parse: 0,
                url: magnet,
                header: {}
        ...
        if (kind === 'error') {
            return JSON.stringify({
                ...
                url: '',
                header: {},
                msg: payload.message || '源站暂无可播放资源'
            });
        }

        // 直连解析（auto / variant）
        if (kind === 'auto' || kind === 'variant') {
            const code = cleanText(payload.code);
            if (!code) {
                return JSON.stringify({ parse: 0, url: '', header: {}, msg: '缺少番号' });
            }

            // 调用解析接口
            // 实际解析路径可能因源站变化，这里尝试常见接口
            try {
                const resolveData = await apiGet('/v1/movies/play', {
                    number: code
                }, 0);

                // 尝试从多种可能字段取播放地址
                let playUrl = '';
                if (resolveData.url) playUrl = resolveData.url;
                else if (resolveData.play_url) playUrl = resolveData.play_url;
                else if (resolveData.m3u8) playUrl = resolveData.m3u8;
                else if (Array.isArray(resolveData.variants) && resolveData.variants.length) {
                    // 按模式排序
                    const variants = resolveData.variants;
                    if (payload.mode === 'speed') {
                        // 简单取第一个
                        playUrl = variants[0].url || variants[0].play_url || '';
                    } else {
                        // 画质优先：找最高
                        let best = variants[0];
                        for (const v of variants) {
                            const h = numberVal(v.height || v.quality);
                            if (h > numberVal(best.height || best.quality)) best = v;
                        }
                        playUrl = best.url || best.play_url || '';
                    }
                } else if (resolveData.data) {
                    const d = resolveData.data;
                    playUrl = d.url || d.play_url || d.m3u8 || '';
                }
            ...
            // 解析失败兜底：尝试用番号搜索详情页再提取（简化）
            return JSON.stringify({
                parse: 0,
                url: '',
                header: {},
                msg: '直连解析失败，请尝试磁力线路或稍后重试'
            });
        }
