...
async function detail(vodIdRaw) {
    try {
        ...
        for (let i = 0; i < 80; i++) {
            ...
            // 去掉?type=json，直接原始接口；播放器访问会自动处理302重定向
            playList.push(`${title}$yujn_play$${apiUrl}@@${randSalt}_${i}`);
        }
        const vod = {
...
async function play(flag, id, flags) {
    try {
        if (id.startsWith("yujn_play$")) {
            ...
            // ----------------核心改动----------------
            // JS层不再发起请求，直接把原始api丢给播放器，parse=1交给播放器内部处理302重定向
            console.log("直接交付播放器处理接口：", realApi);
            return JSON.stringify({
                parse: 1,
                url: realApi,
