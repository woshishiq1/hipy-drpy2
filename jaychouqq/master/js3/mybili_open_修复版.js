...
async function homeVod() {
  try {
    ...
    let body = JSON.parse(await request(html));
    let list = (body.data && body.data.list) || [];
    let videos = [];
    list.forEach(function(it) {
        videos.push({
            ...
            vod_remarks: it.vt_display ? ('🔥 ' + it.vt_display) : (fmtDur(it.duration) || ''),
        });
    });
    return JSON.stringify({
        list: videos,
...
async function category(tid, pg, filter, extend) {
  try {
    extend = extend || {};
    let html = HOST + '/x/web-interface/search/type?search_type=video&page=' + pg + '&keyword=' + encodeURIComponent(extend.tid || tid) + '&duration=' + (extend.duration || '') + '&order=' + (extend.order || '');
    let body = JSON.parse(await request(html));
    if (!body.data || !body.data.result || (body.code !== undefined && body.code !== 0)) {
    ...
    data.result.forEach(function(it) {
        videos.push({
            ...
            vod_name: stripHtmlTag(it.title),
            vod_pic: fixPic(it.pic),
            vod_remarks: fmtDur(it.duration) || '',
        });
    });
    return JSON.stringify({
...
async function detail(id) {
  try {
    let body = JSON.parse(await request(HOST + '/x/web-interface/view?aid=' + id));
    let data = body.data;
    ...
    let vod = {
        ...
        vod_remarks: fmtDur(data.duration) || '',
    ...
    episodes.forEach(function(it, idx) {
        let cid = it.cid;
        let part = String(it.part || ('第' + (idx + 1) + '集')).replace(/#/g, '﹟').replace(/\$/g, '﹩');
...
async function play(flag, id, flags) {
  try {
    ...
    let qns = [116, 80, 64, 32, 16];
    let purl = '';
    for (let qi = 0; qi < qns.length; qi++) {
        let html = HOST + '/x/player/playurl?avid=' + ids[0] + '&cid=' + ids[1] + '&qn=' + qns[qi];
        let body = JSON.parse(await request(html));
        if (body.code !== undefined && body.code !== 0) continue;
        let durl = body.data && body.data.durl;
        if (!durl || durl.length === 0) continue;
        let maxSize = -1;
        let position = -1;
        durl.forEach(function(it, i) {
...
async function search(wd, quick, pg) {
  try {
    ...
    let html = HOST + '/x/web-interface/search/type?search_type=video&keyword=' + encodeURIComponent(wd || '') + '&page=' + pg;
    let body = JSON.parse(await request(html));
    if (!body.data || !body.data.result || (body.code !== undefined && body.code !== 0)) {
...
function fmtDur(duration) {
  ...
  if (typeof duration === 'number') {
      ...
      if (parts.length === 2) {
          totalSec = Number(parts[0]) * 60 + Number(parts[1]);
      } else if (parts.length === 3) {
          totalSec = Number(parts[0]) * 3600 + Number(parts[1]) * 60 + Number(parts[2]);
      } else {
          let n = Number(duration);
          totalSec = isNaN(n) ? 0 : Math.floor(n);
      }
  } else {
      return '';
  }
...
function fixPic(pic) {
  ...
  if (/^http:\/\//i.test(p)) return 'https://' + p.slice(7);
  return p;
}
