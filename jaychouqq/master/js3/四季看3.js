...
/**
 * 返回完整res对象 {content,headers,status}，不再只返回文本，用来捕获302重定向Location
 */
async function request(url,optHeaders={},body){
    try{
        ...
        const res=await req(url,{
            ...
            timeout:12000,
            data:body,
            redirect:0 //关闭自动跟随重定向，可以拿到302的headers.Location
        });
        return res;
    }catch(e){
...
function isVideoUrl(s){
    ...
    return /\.(mp4|m3u8|m4v|flv)(\?|#|$)/.test(s);
}
...
async function detail(vodIdRaw){
    try{
        ...
        if(obj.type==="m3u"||obj.type==="txt"){
            ...
            const res=await request(apiUrl,{Referer:obj.referer||""});
            const playItems=parseLiveText(res.content||"");
...
async function play(flag,id,flags){
    try{
        ...
        //增加随机时间戳，防缓存
        const realReqUrl = fillTimeUrl(apiUrl) + (apiUrl.includes("?")?"&":"?")+"_t="+Date.now();
        const res=await request(realReqUrl);

        let realResourceUrl="";
        //【优先级1：捕获302重定向Location头】yujn接口大多返回302
        if(res&&res.headers){
            let loc = res.headers.Location || res.headers.location;
            if(loc && isHttpUrl(loc)){
                realResourceUrl = text(loc);
                console.log("play:获取302重定向地址",realResourceUrl);
            }
        }

        //【优先级2：JSON解析】
        if(!isHttpUrl(realResourceUrl)){
            const jsonData=safeJson(res.content||"");
            if(jsonData){
                realResourceUrl=text(jsonData.url||jsonData.video||jsonData.data||jsonData.msg||"");
            }
        }

        //【优先级3：正则从返回文本抓取http视频链接】
        if(!isVideoUrl(realResourceUrl)){
            const m=(res.content||"").match(/https?:\/\/[^"'<> \n]+\.(mp4|m3u8)[^"'<> \n]*/g);
            if(m&&m.length>0){
                realResourceUrl=text(m[0]);
            }
        }
        ...
        if(mediaType==="pic"){
            ...
            //视频模式，校验必须是视频后缀
            if(isHttpUrl(realResourceUrl) && isVideoUrl(realResourceUrl)){
                return JSON.stringify({
                    parse:0,
                    url:realResourceUrl,
                    header:DEFAULT_HEADERS
                ...
                console.error("play:未提取有效视频地址",realResourceUrl);
                return JSON.stringify({parse:1,url:"",header:DEFAULT_HEADERS});
            }
        }
    }catch(e){
