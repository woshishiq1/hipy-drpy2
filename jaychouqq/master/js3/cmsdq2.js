//聚合平台3.js【最终修复版】兼容旧CAT引擎，修复中文[]链接，修复双问号bug
...
/**
 * 兼容旧CAT：不使用new URL，转义路径内中文、[]，保留query部分原样
 * @param {string} rawUrl
 * @returns {string}
 */
function fixUrlSpecialChar(rawUrl) {
    ...
    if (qPos >= 0) {
        pathPart = rawUrl.substring(0, qPos);
        queryPart = rawUrl.substring(qPos);
    } else {
        pathPart = rawUrl;
    }
    //只转义路径段 [ ] 中文
    pathPart = pathPart.replace(/\[/g, "%5B").replace(/\]/g, "%5D");
    //中文encodeURI
...
/**
 * 安全追加查询参数，避免双问号
 * @param {string} baseUrl 原始链接，可能已经带?xxx
 * @param {Object} add 要追加的键值对 {ac:"detail",pg:1}
 * @returns {string}
 */
function appendQuery(baseUrl, add) {
    if (!add || Object.keys(add).length === 0) return baseUrl;
    let qPos = baseUrl.indexOf("?");
    let sep = qPos >= 0 ? "&" : "?";
    let parts = [];
    for (let k in add) {
        if(add[k] !== undefined && add[k] !== null){
            parts.push(k + "=" + encodeURIComponent(String(add[k])));
        }
    }
...
async function loadSourceFilter(sourceKey, sourceApi) {
    const url = appendQuery(sourceApi, { ac: "list" });
...
async function category(tid, pg, filter, ext) {
    ...
    try {
        ...
        let paramObj = { ac: "detail", pg: pg };
        if (cateId) paramObj.t = cateId;
        const url = appendQuery(sourceObj.api, paramObj);
        const html = await request(url);
        const data = safeJson(html) || {};
...
async function searchOne(sourceKey, sourceObj, keyword, pg) {
    const url = appendQuery(sourceObj.api, {
        ac: "detail",
        wd: keyword,
        pg: pg
    });
