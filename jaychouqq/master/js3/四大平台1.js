...
function b64DecodeUtf8(b64) {
    try {
        ...
        console.error("b64 decode error", e.message);
        return "";
    }
}
...
async function detail(vodId) {
    try {
        ...
        if (!jo || !Array.isArray(jo.list) || jo.list.length === 0) {
            console.log("detail: api return empty list");
            return JSON.stringify({ list: [] });
        }
        ...
        // -------------------修复点：不再二次分割重组字符串，直接逐段base64编码每一集的url，防止#/$污染--------------------
        if (!vod.vod_play_url || !vod.vod_play_from) {
            console.log("detail: vod_play_url empty");
            return JSON.stringify({ list: [vod] });
        }
        const fromGroups = vod.vod_play_from.split("$$$");
        const urlGroups = vod.vod_play_url.split("$$$");
        const newUrlGroups = [];
        for (let gIdx = 0; gIdx < urlGroups.length; gIdx++) {
            const epStrArr = urlGroups[gIdx].split("#");
            const outEpArr = [];
            for (const epItem of epStrArr) {
                const dollarIndex = epItem.lastIndexOf("$");
                if (dollarIndex <= 0) continue;
                const epName = epItem.substring(0, dollarIndex);
                const rawUrl = epItem.substring(dollarIndex + 1);
                if (!rawUrl) continue;
                outEpArr.push(`${epName}$${b64EncodeUtf8(rawUrl)}`);
            }
            newUrlGroups.push(outEpArr.join("#"));
        }
        vod.vod_play_url = newUrlGroups.join("$$$");
        return JSON.stringify({ list: [vod] });
    } catch (e) {
...
async function play(flag, id, flags) {
    try {
        ...
        if (!rawVideoUrl) {
            console.log("play: rawVideoUrl empty");
        ...
        // 3. 获取webview解析地址，返回parse:1交给壳子webview解析兜底
