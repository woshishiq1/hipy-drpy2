...
async function play(flag, playId, flags) {
    try {
        ...
        if (payload.kind === 'auto' || payload.kind === 'variant') {
            ...
            const resolveData = await apiGet('/v1/movies/play', { number: code }, 0);
            let playUrl = resolveData.url || resolveData.play_url || resolveData.m3u8 || '';
            const variants = resolveData.variants || (resolveData.data && resolveData.data.variants) || [];
            if (!playUrl && Array.isArray(variants) && variants.length) {
                let best = variants[0];
                if (payload.mode !== 'speed') {
                    for (const v of variants) {
                        if (numberVal(v.height || v.quality) > numberVal(best.height || best.quality)) best = v;
                    }
                }
                playUrl = best.url || best.play_url || '';
            }
            if (!playUrl && resolveData.data) {
                playUrl = resolveData.data.url || resolveData.data.play_url || resolveData.data.m3u8 || '';
            }
            if (playUrl) {
                return JSON.stringify({
                    parse: 0,
                    url: playUrl,
                    header: { 'User-Agent': UA, Referer: SOURCE_ORIGIN, Origin: SOURCE_ORIGIN }
                });
            }
