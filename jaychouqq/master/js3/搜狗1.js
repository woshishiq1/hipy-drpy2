...
function safeJson(str) {
    try {
        ...
        console.error("safeJson parse fail", e.message);
        return null;
    }
}
...
async function homeVod() {
    try {
        //首页拿电视剧第一页做首页推荐
        const start = 0;
        let url = `${HOST}/napi/video/classlist?abtest=0&iploc=CN1304&spver=&listTab=teleplay&filter=&start=${start}&len=20&fr=filter`;
        const resp = await request(url);
        const json = safeJson(resp);
        if(!json) return JSON.stringify({ list: [] });
        let results = [];
        if(json.listData && Array.isArray(json.listData.results)){
            results = json.listData.results;
        }
        const list = [];
...
async function category(tid, pg, filter, ext) {
    ...
    try {
        ...
        const queryArr = [];
        //修复：只追加非空筛选参数，避免多余&符号
        if(ext && ext.style) queryArr.push(`style=${encodeURIComponent(ext.style)}`);
        if(ext && ext.zone) queryArr.push(`zone=${encodeURIComponent(ext.zone)}`);
        if(ext && ext.year) queryArr.push(`year=${encodeURIComponent(ext.year)}`);
        if(ext && ext.fee) queryArr.push(`fee=${encodeURIComponent(ext.fee)}`);
        if(ext && ext.order) queryArr.push(`order=${encodeURIComponent(ext.order)}`);
        if(queryArr.length>0) url += "&" + queryArr.join("&");

        const resp = await request(url);
        const json = safeJson(resp);
        let results = [];
        //兼容多种返回层级，防止拿不到results直接空列表
        if(json){
            if(json.listData && Array.isArray(json.listData.results)){
                results = json.listData.results;
...
async function search(key, quick, pg) {
    ...
    try {
        ...
        const resp = await request(url);
        //修复正则：多行、空格、换行兼容
        const reg = /INITIAL_STATE\s*=\s*(\{[\s\S]*?\});/;
        let match = resp.match(reg);
        if(!match){
            console.error("search INITIAL_STATE regex not match");
            return JSON.stringify({ list:[], page:pg, pagecount:0, land:1, ratio:1.33 });
        }
        const json = safeJson(match[1]);
        let results = [];
        if(json && json.result && json.result.resultData && json.result.resultData.searchData && Array.isArray(json.result.resultData.searchData.results)){
            results = json.result.resultData.searchData.results;
        }
        const list = [];
        for(const it of results){
            ...
            list.push({
                ...
                vod_pic: fixPicUrl(it.v_picurl||""),
                vod_remarks: Array.isArray(it.listCategory) ? it.listCategory.join(",") : "",
                style:{type:"rect",ratio:1.33}
            });
        }
        return JSON.stringify({
...
async function detail(vodId) {
    try {
        ...
        if(!match) {
            console.error("detail INITIAL_STATE regex not match");
            return JSON.stringify({ list:[] });
        }
        const json = safeJson(match[1]);
        ...
        const vod = {
            vod_id: vodId,
            vod_name: jd.name || "",
            vod_pic: fixPicUrl((jd.photo && jd.photo.item_list && jd.photo.item_list[0]) || ""),
            vod_year: "",
            vod_area: jd.zone || "",
