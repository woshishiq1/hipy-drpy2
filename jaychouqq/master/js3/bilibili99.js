...
// ==========脚本内置默认配置（写死在JS内部）==========
const BUILT_IN = {
    categories: "音乐#动画#电影#电视剧#番剧#综艺#影视#国创#娱乐#游戏#搞笑#单机游戏#沙雕动画#沙雕穿越#沙雕",
    cookie: "DedeUserID=690781341;DedeUserID__ckMd5=cabc96906269c5b6;;          "
};
...
async function request(reqUrl, ua, buffer) {
    try {
        let res = await req(reqUrl, {
            method: 'get',
            headers: ua ? ua : { 'User-Agent': UA, Referer: 'https://www.bilibili.com/' },
            timeout: 60000,
            buffer: buffer ? 1 : 0,
        });
        return res?.content ?? '';
...
async function init(cfg) {
    try {
        siteKey = cfg.skey;
        siteType = cfg.stype;
        // 优先级：源ext > JS内置BUILT_IN
        let ext = cfg.ext || {};
        // 取分类
        let catRaw;
        if (ext?.categories) {
            catRaw = ext.categories;
        } else {
            catRaw = BUILT_IN.categories;
        }
        let catList = [];
        if (Array.isArray(catRaw)) {
            catList = [...catRaw];
        } else {
            catList = String(catRaw || '').split('#');
        }
        //取cookie
        if (ext?.cookie && String(ext.cookie).trim() !== '') {
            cookie = String(ext.cookie).trim();
        } else {
            cookie = String(BUILT_IN.cookie).trim();
        }
        ...
        for (const kk of catList) {
            const name = String(kk).trim();
            if (!name) continue;
            newarr.push({ type_name: name, type_id: name, land: 1, ratio: 1.33 });
            d[name] = jsonData;
        }
...
async function category(tid, page, filter, ext) {
    ...
    try {
        ...
        let pagecount = 1;
        if (data?.numPages) pagecount = data.numPages;
        else if (videos.length > 0) pagecount = page + 1;
        ...
        if (flag === 'dash' || flag === '相关') {
            ...
            for (let i = 0; i < qualityIds.length; i++) {
                ...
                for (const dashjson of video) {
                    if (String(dashjson.quality) === String(qualityIds[i])) {
...
async function search(key, quick, pg) {
    ...
    if (page <= 0) page = 1;
    try {
        const url = `https://api.bilibili.com/x/web-interface/search/type?search_type=video&keyword=${encodeURIComponent(key)}&page=${page}&order=0&duration=0&tids_1=0`;
        const respText = await request(url, getHeaders());
        ...
        let pagecount = 1;
        if (json?.data?.numPages) pagecount = json.data.numPages;
        else if (videos.length > 0) pagecount = page + 1;
        return JSON.stringify({
            page: page,
            pagecount: pagecount,
...
async function proxy(segments, headers) {
    try {
        ...
        if (what === 'dash') {
            ...
            const aid = ids[0];
            const cid = ids[1];
            const targetQn = ids[2];
            const apiUrl = `https://api.bilibili.com/x/player/playurl?avid=${aid}&cid=${cid}&qn=${targetQn}&fnval=4048&fourk=1`;
            const respJson = safeJson(await request(apiUrl, getHeaders()));
            const dash = respJson?.data?.dash;
            if (!dash) throw new Error('dash empty');
            let videoList = '';
            let audioList = '';
            for (const dashjson of dash.video ?? []) {
                if (String(dashjson.quality) === String(targetQn)) {
