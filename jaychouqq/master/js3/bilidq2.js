...
async function category(tid, pg, filter, ext) {
    ...
    try {
        const keyword = tid;
        const ps = 20;
        const url = `https://api.bilibili.com/x/web-interface/wsearch?keyword=${encodeURIComponent(keyword)}&page=${pg}&ps=${ps}`;
        const resp = await request(url);
        const json = safeJson(resp);
        if (!json || json.code !== 0) {
            return JSON.stringify({ list: [], page: pg, pagecount: 0 });
        }
        const videoArr = json?.data?.video || [];
        const list = [];
        ...
        const pagecount = list.length > 0 ? pg + 1 : 1;
...
async function search(key, quick, pg) {
    ...
    try {
        const ps = 20;
        const url = `https://api.bilibili.com/x/web-interface/wsearch?keyword=${encodeURIComponent(key)}&page=${pg}&ps=${ps}`;
        const resp = await request(url);
        const json = safeJson(resp);
        if (!json || json.code !== 0) {
...
async function detail(aidStr) {
    try {
        ...
        const resp = await request(url);
        const json = safeJson(resp);
        if (!json || json.code !==0) return JSON.stringify({ list: [] });
        ...
        let qualityList = [];
        if(login && vip){
            qualityList = [127,120,116,112,80,64,32,16];
        }else if(login && !vip){
            qualityList = [112,80,64,32,16];
        }else{
            qualityList = [80,64,32,16];
        }
        let descList = [];
        for(let q of qualityList){
            descList.push(b64EncodeUtf8(QUALITY_NAME_MAP[q]||`${q}P`));
        }
        ...
        for (let j = 0; j < pages.length; j++) {
            ...
            playList.push(`${partTitle}$${aid}+${cid}+${qualityList.join(':')}+${descList.join(':')}`);
        }
...
async function play(flag, id, flags) {
    try {
        ...
        for (let i = 0; i < qualityIds.length; i++) {
            const qn = parseInt(qualityIds[i]);
            ...
            if(!respJson || respJson.code !==0) continue;
            const data = respJson?.data;
            if (!data || !data.durl || !data.durl[0]) continue;
            if (data.quality !== qn) continue;
            urls.push(b64DecodeUtf8(qualityName[i] || ''), data.durl[0].url);
        }
        if(urls.length ===0){
            return JSON.stringify({
                parse:1,
