...
async function detail(vodId) {
    try {
        ...
        let realHls = "";
        // 【修复】优先读取video数组，按码率从大到小取可用m3u8
        if(Array.isArray(jo.video) && jo.video.length>0){
            // 码率降序排序
            const videoArr = [...jo.video].sort((a,b)=>(b.bitrate||0)-(a.bitrate||0));
        ...
        const vod = {
            ...
            vod_content:"",
            vod_play_from:"CCTV",
            vod_play_url: `${title}$${b64EncodeUtf8(realHls)}`
        };
...
async function play(flag, id, flags) {
    try {
        ...
        // 【修复】删除强制修改2000码率逻辑，直接返回原始地址，不再强行替换
        return JSON.stringify({
            parse:0,
            url: raw,
