...
async function homeVod() {
  ...
  let resp;
  try {
      resp = JSON.parse(await request(html));
  } catch (e) {
  ...
  (data.list || []).forEach(function(it) {
      videos.push({
          ...
          vod_remarks: (it.vt_display ? '🔥 ' + it.vt_display : (secToDHM(it.duration) || '')),
      });
  });
  return JSON.stringify({
      list: videos,
...
async function category(tid, pg, filter, extend) {
  extend = extend || {};
  let kw = extend.tid || tid || '';
  let url = HOST + '/x/web-interface/search/type?search_type=video&page=' + pg
      + '&keyword=' + encodeURIComponent(kw)
      + '&duration=' + encodeURIComponent(extend.duration || '')
      + '&order=' + encodeURIComponent(extend.order || '');
  let resp;
  try {
      resp = JSON.parse(await request(url));
  } catch (e) {
  ...
  return JSON.stringify({
      ...
      pagecount: parseInt(data.numPages) || 1,
      limit: 20,
      total: data.numResults || videos.length,
      list: videos,
  });
}
...
async function detail(id) {
  let resp;
  try {
      resp = JSON.parse(await request(HOST + '/x/web-interface/view?aid=' + id));
  } catch (e) {
      return JSON.stringify({ list: [] });
  }
  let data = resp.data || {};
  ...
  let vod = {
      ...
      vod_year: data.pubdate ? new Date(data.pubdate * 1000).getFullYear() : '',
      vod_remarks: secToDHM(data.duration),
      vod_director: (data.owner && data.owner.name) || '',
      vod_content: stripHtmlTag(data.desc),
  };
  let episodes = data.pages || [];
  let playurls = [];
  episodes.forEach(function(it, idx) {
    let cid = it.cid;
    let part = (it.part || '')
        .replace(/#/g, '﹟')
        .replace(/\$/g, '﹩');
    if (!part.trim()) part = '第' + (idx + 1) + '集';
    playurls.push(
        part + '$' + data.aid + '_' + cid
    )
  });
...
async function play(flag, id, flags) {
  ...
  let aid = ids[0], cid = ids[1];
  if (!aid || !cid) {
      return JSON.stringify({ parse: 0, url: '', header: getMb() });
  }
  // qn 从高到低尝试：116(1080P高帧率,需大会员) -> 80(1080P) -> 64(720P)
  let qnList = ['116', '80', '64'];
  let purl = '';
  for (let i = 0; i < qnList.length; i++) {
      try {
          let resp = JSON.parse(await request(HOST + '/x/player/playurl?avid=' + aid + '&cid=' + cid + '&qn=' + qnList[i]));
          let data = resp.data || {};
          let durl = data.durl || [];
          if (durl.length === 0) continue; // 本档没拿到,降级
          let maxSize = -1;
          let position = -1;
          durl.forEach(function(it, idx) {
...
async function search(wd, quick, pg) {
  ...
  let url = HOST + '/x/web-interface/search/type?search_type=video&keyword=' + encodeURIComponent(wd) + '&page=' + pg;
  let resp;
  try {
      resp = JSON.parse(await request(url));
  } catch (e) {
      return JSON.stringify({ page: 1, pagecount: 1, limit: 20, total: 0, list: [], land: 1, ratio: 1.78 });
  }
  let data = resp.data || {};
  let videos = [];
  (data.result || []).forEach(function(it) {
...
// 兼容 B站两种时长返回：纯数字(秒) 或 "mm:ss" / "h:mm:ss"
function turnDHM(duration) {
  if (duration === null || duration === undefined || duration === '') return null;
  let min = 0, sec = 0;
  if (typeof duration === 'number' || /^\d+$/.test(String(duration))) {
      ...
      if (parts.length === 3) {
          // h:mm:ss
          min = Number(parts[0]) * 60 + Number(parts[1]);
          sec = Number(parts[2]);
      } else {
          min = Number(parts[0]);
...
// 补全图片协议前缀（B站多数接口返回 //i0.hdslb.com 开头）
function fixPic(pic) {
  if (!pic) return '';
...
// 纯秒数 -> 可读时长（detail 用, B站 view 返回秒）
function secToDHM(sec) {
  if (sec === null || sec === undefined || sec === '') return '';
  return turnDHM(Number(sec)) || '';
}
